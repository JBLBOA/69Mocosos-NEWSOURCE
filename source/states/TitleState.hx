...
import soundTest.SoundTestMenu;
import flixel.input.keyboard.FlxKey;
...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		var bg:FlxBackdrop = new FlxBackdrop(Paths.image('backdropmenu'));
		...
		logoBl.frames = Paths.getSparrowAtlas('LOGO_ANIMADO_69');
		logoBl.antialiasing = ClientPrefs.data.antialiasing;
		logoBl.scale.set(0.7,0.7);
  
		logoBl.animation.addByPrefix('bump', 'LOGO ANIMADO 69', 24, false);
		...
		//add(gfDance); FUCK NIGGERS I FUCKING HATE NIGGERS I HATE NIGGERS FUCK NIGGERS I HATE EVERYNIGGERS ARE IN THE WORLD
		logoBl.screenCenter();
		add(logoBl);
		if(swagShader != null)
		{
		...
		gian = new FlxSprite().loadGraphic(Paths.image('gianBACKDROP'));
		gian.alpha = 0.0;
		gian.screenCenter();
		add(gian);
		...
		ngSpr = new FlxSprite(0, FlxG.height * 0.52).loadGraphic(Paths.image('gianlogo'));
		add(ngSpr);
		ngSpr.visible = false;
	...
	override function update(elapsed:Float)
	{
		...
		if (initialized && !transitioning && skippedIntro)
		{
			...
			if(pressedEnter)
			{
				...
				FlxG.sound.play(Paths.sound('mierda'), 1);

				gian.alpha = 1.0;
				gian.scale.x = 2.5;
				gian.scale.y = 1.5;

				FlxTween.tween(gian.scale, {x: 0.0, y: 0.0 }, 5);
				FlxTween.tween(gian, {alpha: 0.0 }, 5);
				...
				if(allowedKeys.contains(keyName)) {
					...
					for (wordRaw in easterEggKeys)
					{
						...
						if (easterEggKeysBuffer.contains(word))
						{
							...
							FlxTween.tween(black, {alpha: 1}, 1, {onate.skipNextTransIn = true;
									FlxTransitionableState.skipNextTransOut = true;
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				case 2:
					createCoolText(['CP TEAM'], 40);
				case 4:
					addMoreText('PRESENTA');
				case 5:
					deleteCoolText();
				case 6:
					createCoolText(['NO ASOCIADO', 'CON'], -40);
				case 8:
					addMoreText('VS GIAN', -40);
					ngSpr.visible = true;
				case 9:
					deleteCoolText();
					ngSpr.visible = false;
